    ...
    <div className="fixed bottom-5 right-5 z-50">
      {chatOpen ? (
        <div className="w-[min(92vw,34rem)] rounded-3xl border border-border bg-slate-50 p-4 shadow-2xl">
            ...
            {relatedProducts.length ? (
              <div className="mt-2 grid grid-cols-1 gap-2">
                {relatedProducts.slice(0, 4).map((product, index) => (
                  ...
                  >
                    <img
                      src={product.thumbnail_url || "https://images.unsplash.com/photo-1517336714739-489689fd1ca8?w=120"}
          ...
          <div className="mb-3 flex flex-wrap gap-2">{config.aiSuggestions.slice(0,2).map((s) => <button key={s} onClick={() => setValue(s)} className="rounded-full bg-slate-200 px-3 py-1.5 text-xs">{s}</button>)}</div>
          {mounted && token ? (
            <form onSubmit={submit} className="flex gap-2">
              ...
              <button className="rounded-xl bg-slate-900 px-4 text-sm text-white">Gửi</button>
            </form>
          ) : (
